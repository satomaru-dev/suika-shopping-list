...
export default function ShoppingApp() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {tab === "list" && <section>
        <div className="hero-card"><p className="eyebrow light">声でも、指でも</p><h2>なにを買う？</h2><div className="voice-control"><button className={`mic-button ${listening ? "listening" : ""}`} onClick={startVoice} aria-label="音声入力"><Image className="mic-watermelon" src="/icons/icon-192.png" alt="" width={64} height={64} priority />{listening && <MicOff className="listening-mark" />}</button><span>{listening ? "聞いています…" : "スイカを押して話す"}</span></div><div className="add-row"><input value={input} onChange={(e) => { setInput(e.target.value); setInputSource("web"); }} onKeyDown={(e) => { if (e.key === "Enter") void addCurrent(); }} placeholder="牛乳、卵、パン" aria-label="追加する商品" /></div>
